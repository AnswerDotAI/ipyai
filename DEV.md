...
- **Remote gateways.** All the machinery accepts a URL (`IPYAI_GATEWAY`), but remote use needs: the shell↔kernel cwd sync switched off (shell local, kernel remote — bare-`!` local vs embedded-`!` remote must be allowed to disagree); token resolution shared with clikernel's `gateways.toml`; and `#ai`/media refs resolving against the wrong filesystem. Local-only until designed.
